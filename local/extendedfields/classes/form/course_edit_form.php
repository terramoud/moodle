...
namespace local_extendedfields\form;
...
class course_edit_form extends \moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('header', 'extendedfields_h', 'Custom Fields');

        $locations = $DB->get_records_select_menu('mcd_ef_locations', '', [], 'id', 'id,location');
        $options = array(                                                                                                           
            'multiple' => false,                                                  
            'noselectionstring' => 'Select or create new location',                                                                
            'tags' => true,                                                                
        );
        $mform->addElement('autocomplete', 'extendedfields[location_id]', 'Location', [""] + $locations, $options);

        $location_addresses = $DB->get_records_select_menu('mcd_ef_location_addr', '', [], 'id', 'id,location_address');
        $options = array(                                                                                                           
            'multiple' => false,                                                  
            'noselectionstring' => 'Select or create new location address',                                                                
            'tags' => true,                                                                
        );
        $mform->addElement('autocomplete', 'extendedfields[location_address_id]', 'Location address', [""] + $location_addresses, $options);


        $currencies = array_map('trim', explode("\n", get_config('local_extendedfields', 'currencies')));
        $mform->addElement('select', 'extendedfields[currency]', 'Currency', array_combine($currencies, $currencies));

        $mform->addElement('float', 'extendedfields[tuition_fee]', 'Tuition fee');
        $mform->addElement('float', 'extendedfields[service_fee]', 'Service fee');
        $mform->addElement('float', 'extendedfields[total_cost]', 'Total cost');

        $mform->addElement('advcheckbox', 'extendedfields[hide_price]', 'Hide price');

        $vendors = $DB->get_records_select_menu('mcd_ef_vendors', '', [], 'id', 'id,vendor');
        $options = array(                                                                                                           
            'multiple' => false,                                                  
            'noselectionstring' => 'Select or create new vendor',                                                                
            'tags' => true,                                                                
        );
        $mform->addElement('autocomplete', 'extendedfields[vendor_id]', 'Vendor', [""] + $vendors, $options);
        $mform->addElement('textarea','extendedfields[vendor_contact_info]', 'Vendor contact info', 'wrap="virtual" rows="5" cols="50"');

        $mform->addElement('float', 'extendedfields[max_students]', 'Maximum number of students');
        $mform->addElement('duration', 'extendedfields[duration]', 'Duration');

        $users = $DB->get_records_select_menu('user', 'suspended = 0', [], 'id', 'id,CONCAT(firstname, \' \', lastname)');
        $options = array(                                                                                                           
            'multiple' => false,                                                  
            'noselectionstring' => 'Select admin user',                                                                
        );
        $mform->addElement('autocomplete', 'extendedfields[admin_user_id]', 'Admin user', [""] + $users, $options);
        $mform->setDefault('extendedfields[admin_user_id]', $USER->id);

        $options = array(                                                                                                           
            'multiple' => true,                                                  
            'noselectionstring' => 'Select internal instructors or save custom text',                                                                
            'tags' => true,                                                                
        );
        $mform->addElement('autocomplete', 'extendedfields[internal_instructors]', 'Internal instructors', [""] + $users, $options);
        
        $mform->addElement('advcheckbox', 'extendedfields[mandatory]', 'Mandatory');
        $mform->addElement('duration', 'extendedfields[mandatory_exp_period]', 'Mandatory Expiration period');
        $mform->disabledIf('extendedfields[mandatory_exp_period]', 'extendedfields[mandatory]');
    ...
    public function set_data($data) {
        global $USER;
        $extendedfields = (array)array_values($this->_customdata['extendedfields'])[0];

        foreach ($extendedfields as $field => $fielddata) {
            $data->extendedfields[$field] = $fielddata;
        }
    ...
    function validation($data, $files) {
        ...
        foreach ($number_fields as $field) {
            if (!empty($data['extendedfields'][$field]) && !is_int($data['extendedfields'][$field])) {
                    $errors['extendedfields['.$field.']'] = "You must enter a number without decimals here.";
                }
        }
